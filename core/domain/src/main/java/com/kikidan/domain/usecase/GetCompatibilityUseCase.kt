...
class GetCompatibilityUseCase
    @Inject
    constructor(
        private val compatibilityRepository: CompatibilityRepository,
    ) {
        suspend operator fun invoke(compatibilityId: String): Result<Compatibility> =
            compatibilityRepository.getCompatibility(compatibilityId)
    }
